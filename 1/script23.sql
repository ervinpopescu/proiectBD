...
CREATE DATABASE proiectBD COLLATE 'utf8_unicode_ci';
USE proiectBD;
...
CREATE TABLE tblLocatieMasina (
    idLocatieMasina TINYINT(4) ZEROFILL UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    x FLOAT(10) ZEROFILL UNSIGNED NOT NULL,
    y FLOAT(10) ZEROFILL UNSIGNED NOT NULL
);
...
INSERT INTO tblLocatieMasina VALUES(DEFAULT, 44.433446, 26.057620);
...
alter table `tblclient` add `created_at` timestamp null, add `updated_at` timestamp null;
alter table `tblmasina` add `created_at` timestamp null, add `updated_at` timestamp null;
alter table `tbllocatie` add `created_at` timestamp null, add `updated_at` timestamp null;
alter table `tbllocatiemasina` add `created_at` timestamp null, add `updated_at` timestamp null;
alter table `tblinchiriere` add `created_at` timestamp null, add `updated_at` timestamp null;
alter table `tblplata` add `created_at` timestamp null, add `updated_at` timestamp null;

-- DESCRIBE tblClient;
-- DESCRIBE tblMasina;
-- DESCRIBE tblLocatie;
-- DESCRIBE tblLocatieMasina;
